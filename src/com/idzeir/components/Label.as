package com.idzeir.components
{
	...
	public class Label extends TextField
	{
		...
		/**
		 * 单行显示文本，超出限制长度自动显示为带"..." 
		 * @param content 指定文本显示内容
		 * @param color 文本颜色
		 * @param useHtml 构造文本是否为html富文本
		 * @param maxW 文本限制宽度
		 */	
		public function Label(content:String = "", color:int = 0x000000, useHtml:Boolean = false, maxW:Number = 80):void
		{
			this.autoSize="left";
			this._maxWidth=maxW;				
			this._color = color;
			...
			var tf:TextFormat=new TextFormat("Microsoft YaHei,微软雅黑,宋体",12,color,true);
			this.defaultTextFormat=tf;
			useHtml ? htmlText=content : text = content;
		}
